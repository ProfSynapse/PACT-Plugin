...
**When to terminate**: Last resort — agent resumed once and stalled again, looping on same error 3+ times, context exhausted, or TeammateIdle stall unresolved by resume. `TaskStop` is a force-stop (immediate, non-cooperative) and the termination primitive; use it directly. A `shutdown_request` does not terminate the agent — on the tmux backend an approved `shutdown_response` was **observed not to terminate** the pane/process (n=1 pane, 2026-07-12, platform-bug-vs-intended unresolved) and its in-process semantics are **unprobed** — and PACT's shutdown flows no longer send one. Against a **mid-turn** teammate `TaskStop` is **unmeasured on both backends** — and an agent being force-terminated for a stall is very likely mid-turn, so this is the surface where that gap bites hardest. It is not risk-free. "Last resort" scopes the decision to terminate an unrecoverable agent mid-workflow — not the `TaskStop` primitive itself, which is also the routine call in graceful shutdown flows. After termination, spawn a fresh agent with partial HANDOFF from the terminated agent's task metadata.
