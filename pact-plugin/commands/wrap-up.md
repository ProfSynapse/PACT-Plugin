...
5. **Variety divergence**: Was per-dispatch variety distribution materially different from feature variety? **Inputs**: `{plugin_root}` and `{session_dir}` are the values recorded in the Current Session block of `CLAUDE.md`; `feature_task_id` is the id of the feature task this wrap-up is closing. Helpers import from the plugin's hooks directory, so put it on the path first: `import sys; sys.path.insert(0, "{plugin_root}/hooks")`. **Arc scope (current feature only)**: in a resumed/multi-feature session the journal holds prior arcs, so FIRST compute the current arc's start timestamp. Read every `variety_assessed` event — `python3 "{plugin_root}/hooks/shared/session_journal.py" read --type variety_assessed --session-dir '{session_dir}'` — then `from shared.variety_divergence import resolve_arc_start` and `arc_start = resolve_arc_start(variety_assessed_events, feature_task_id)`. The helper returns the LATEST `ts` among events matching `feature_task_id` — the platform reuses task_ids across arcs, so the current feature's id can also match a PRIOR arc, and the latest-ts match is the current arc (this is why a plain `read-last` of ANY feature is wrong) — or `None` when no event matches. Pass `--since '{arc_start}'` on EVERY read below — it compares parsed timestamps, not strings, and INCLUDES an event landing exactly on the boundary; when `arc_start` is `None`, omit `--since` and read the whole journal. **Feature variety**: take it from the `variety_assessed` event whose `ts` equals `arc_start` — `feature_variety = resolve_variety_total(event["variety"])` via `from shared.teachback_schema import resolve_variety_total`. Do not read it from the task store: that store is reaped by the teams/tasks reaper, and the journal copy is already loaded here. When no event matches, `feature_variety` is `None` and the delta is not computed. Read `dispatch_site` events: `python3 "{plugin_root}/hooks/shared/session_journal.py" read --type dispatch_site --since '{arc_start}' --session-dir '{session_dir}'` — **the `read` subcommand prints a SINGLE JSON array**, so `events = json.loads(output)` and iterate the list; do NOT parse line-by-line, and do NOT pipe through `2>/dev/null` / `|| echo` / `head` (they mask a parse crash as emptiness). A read that exits non-zero, prints non-JSON, or names a missing session dir is a FAILED read, not an empty one: report the failure and stop this question rather than treating it as zero events. Then derive BOTH Q5 terms in ONE call via the pure helper `from shared.variety_divergence import extract_dispatch_coverage`: `variety_totals, total_pact_dispatch_count, malformed_stamps = extract_dispatch_coverage(events)`. The denominator is the EXISTENCE of each `dispatch_site` event; the numerator is `len(variety_totals)`, the resolvable stamps within those same events. Both terms come from one pass over one list, so they cannot be sourced over different populations and coverage can never exceed 1.0. Pass that SAME list onward — `compute_variety_divergence(feature_variety, variety_totals, total_pact_dispatch_count)` from `from shared.variety_divergence import compute_variety_divergence` — never a re-derived one, and always with all three arguments: the two-argument form returns a coverage of 1.0 and a `total` of `None` without raising. Report `len(malformed_stamps)` alongside the coverage line: a stamp that is PRESENT but unresolvable is a data-quality defect, whereas an ABSENT stamp is an honest un-stamped dispatch and is the gap coverage exists to surface. They have different remedies, so never merge them. Un-stamped dispatches still count in the denominator, so coverage can sit below 1.0 (e.g. 6/7). **The returned dict** carries `coverage`, `stamped`, `total`, `mean`, `max`, `min`, `delta`, `surfaced`, `direction`, `reason`. `stamped` and `total` are the raw counts the render below requires. `mean` is rounded to an integer, so `[9, 8]` yields `8`. `direction` describes the FEATURE relative to the dispatch mean: `overshot` means the feature variety was scored HIGHER than the dispatches turned out to be, `undershot` LOWER — state it that way round, because the opposite reading inverts the finding. `reason` is `None` on a surfaced divergence, and otherwise one of `no_dispatch_sites`, `zero_coverage`, `feature_variety_missing`, `within_threshold` (the ordinary in-band result) or `coverage_exceeds_unity`. **Zero coverage has TWO opposite meanings and `total` is what tells them apart — never the `coverage` float, which is `0.0` in both.** `reason="no_dispatch_sites"` (total 0) means no dispatch site was recorded: `surfaced=False`, report **N/A, never `0.000`**. `reason="zero_coverage"` (total > 0, nothing stamped) means every dispatch site exists and NONE was stamped — the emit worked and the stamping did not. That one arrives with `surfaced=True` because it is the loudest legitimate reading this metric produces, and suppressing it is an invisible refusal reading as "nothing to report". **Honour `surfaced` and render the raw counts (`stamped` of `total`, e.g. "0 of 5") rather than `coverage` alone** — a consumer that prints the ratio without the counts cannot distinguish the two states, and a confident `0.000` for a session that simply had nothing to measure manufactures a compliance gap out of a missing stream. On the `zero_coverage` path ALSO report `len(malformed_stamps)`: "0 of 5, 5 malformed" is a producer defect while "0 of 5, 0 malformed" means nothing was ever stamped, and the two have completely different remedies. `surfaced` answers whether the DIVERGENCE is material; it does not answer how complete the measurement was. Report a coverage gap whenever `stamped < total`, including when `surfaced` is False. **Denominator liveness (Check A) — run this whenever the coverage read completes, INCLUDING when it reports N/A**, because the case most worth catching is a session that looks like it dispatched nothing. Read two more streams with the SAME `--since '{arc_start}'` scoping and the same single-JSON-array parse: `--type dispatch_decision` and `--type journal_emit_skipped`. Then call the pure helper `from shared.variety_divergence import check_denominator_liveness` together with `from shared.intentional_wait import TEACHBACK_EXEMPT_AGENT_TYPES`, and pass that constant through: `liveness = check_denominator_liveness(events, dispatch_decision_events, skip_events, TEACHBACK_EXEMPT_AGENT_TYPES)`. **Thread the constant rather than writing the exempt set out here** — the witness must exclude exactly what the denominator excludes, and a second copy of that set is how the two silently drift apart. The helper returns `state` plus RAW COUNTS and deliberately no ratio; there is no threshold to tune, and `dispatch_decision` was chosen for being degenerate rather than for any ratio it produces. **Render Check A on its own line ABOVE the coverage line**, so it is visible in exactly the session it exists for. Render by `state`: `alive` → report nothing, it is the ordinary case. `denominator_stream_dead` → report **"denominator stream dead — {dispatch_sites} dispatch sites, {witness_spawns} specialist spawns"** from the raw counts, as an **ADVISORY that never clamps or suppresses the coverage figure**; it means specialists were spawned and NO dispatch site was recorded, i.e. the emit path is broken rather than the stamping. `witness_degraded` → report **"liveness INCONCLUSIVE — the witness itself was dropped this session"** and do **NOT** report liveness as healthy: a check that knows its own witness is lossy must not return "alive". 🔴 **INCONCLUSIVE is a CHECK-A verdict and NEVER a coverage reason — report it alongside the coverage figure, never instead of it.** The two answer different questions over different inputs: coverage is computed from the `dispatch_site` stream alone and never reads `dispatch_decision`, so **a dropped witness must not suppress or qualify a perfectly good coverage ratio.** Letting it would have one stream's degradation silently invalidate a measurement it does not feed — and the state it would swallow is `zero_coverage`, the ONLY reason carrying `surfaced=True` and the loudest legitimate reading this metric produces. **Emit-site skips (Check B)**: from the same `skip_events`, call `from shared.variety_divergence import count_emit_skips` and report `total` with its `by_cause` breakdown whenever `total > 0` — **keep `raised` and `returned_false` separate in what you print**, because a writer defect and a schema rejection have different remedies and a single merged "failed" number rebuilds the ambiguity these events exist to resolve. A skip event and a Check A advisory can describe the SAME lost emit; report each once and do not present them as two independent findings. 🔴 **When any skip carries `skipped_type == "dispatch_site"`, the denominator is missing those sites and the coverage figure is an OVER-estimate.** Qualify the render — "{stamped} of {total} recorded — denominator incomplete" — and do NOT recompute the denominator or fold the lost sites into `total`: the printed arithmetic stays exactly what the helper returned, and the qualification is a caveat beside it, never a change to it. **Neither check is coverage and you must not present them as such**: together they detect TOTAL loss of the site stream plus the two skip paths that record themselves. **Emit loss OUTSIDE those recorded paths is invisible to both**, so a green liveness result means "not obviously dead", never "the denominator is complete" — but loss on the recorded paths is NOT invisible: those skips name their task_ids in `skip_events`, which this question already reads, so report them rather than describing the gap as unknowable. When the `dispatch_site` stream is empty, FIRST apply the **masked-empty guard** — re-read the raw `session-journal.jsonl` and confirm `dispatch_site` is genuinely absent, because error-suppression or a mis-parse can make a crashed read look identical to absence — and only then omit the distribution with the note "Per-dispatch variety not available — no dispatch sites recorded". State what was observed and do not diagnose why: an older session that pre-dates dispatch_site emission and a session whose emit path is broken produce the SAME journal signature, so this branch cannot tell them apart and must not claim to. Check A still runs and still renders on this path, and it errs toward the advisory: a `denominator stream dead` report on a genuinely old session is noise, while a broken emit reported as an absence is silence. See [pact-variety.md §Variety Calibration Record](../protocols/pact-variety.md#variety-calibration-record) for the schema; sample output below.
...
**Sample output for question 5 (variety divergence).** Six filled forms, one per `reason`. Every
slot is a key from the returned dict or a value produced earlier in this question; fill the slots and
print the lines shown, and print no line a form does not list. `coverage_exceeds_unity` has no form
because `extract_dispatch_coverage` cannot produce `stamped > total` — it is unreachable here, not
overlooked. Check A prints its own line ABOVE the form whenever `state` is not `alive`; an `alive`
check prints nothing, and its absence is the ordinary case rather than a missing line.

**ORDINARY** — `reason=None`, `surfaced=True`:
```
**Variety divergence** (question 5):
- Feature variety: {feature_variety}
- Per-dispatch distribution: mean={mean}, min={min}, max={max}
- Coverage: {stamped} of {total} stamped, {len(malformed_stamps)} malformed
- Delta (feature vs mean): {delta} → SURFACED
- Direction: feature {direction} — {direction} describes the FEATURE against the dispatch mean
```

**IN BAND** — `reason="within_threshold"`, `surfaced=False`. Same lines; the delta line says why it
did not surface:
```
**Variety divergence** (question 5):
- Feature variety: {feature_variety}
...
**NOTHING STAMPED** — `reason="zero_coverage"`, `surfaced=True`. `mean`/`min`/`max` are `None`, so the
distribution line is REPLACED rather than filled with nulls:
```
**Variety divergence** (question 5):
- Feature variety: {feature_variety}
- No per-dispatch distribution: nothing was stamped, so mean/min/max do not exist
- Coverage: {stamped} of {total} stamped, {len(malformed_stamps)} malformed
```

**NO FEATURE VARIETY** — `reason="feature_variety_missing"`, `surfaced=False`. `delta` and `direction`
are `None`, so those lines are dropped; the distribution still prints:
```
**Variety divergence** (question 5):
...
**NO DISPATCH SITES** — `reason="no_dispatch_sites"`, `surfaced=False`. One line, and never a zero:
```
**Variety divergence** (question 5): N/A — no dispatch sites recorded this arc
```

**DENOMINATOR INCOMPLETE** — not a `reason`; an OVERLAY on any form above. With
`skips = count_emit_skips(skip_events)`, it applies whenever `skips["by_type"].get("dispatch_site", 0) > 0`.
Replace that form's Coverage line with this one and print the rest of the form unchanged:
```
- Coverage: {stamped} of {total} recorded, {skips[by_type][dispatch_site]} sites lost — denominator incomplete
```

**Check A**, whenever `state` is not `alive`, prints ABOVE the form:
```
- {state} — {dispatch_sites} dispatch sites, {witness_spawns} specialist spawns
```

**Check B**, whenever `skips["total"] > 0`, prints last in every form:
```
- Emit skips: {skips[total]} ({skips[by_cause]})
```
...
decisions: [
  ...
  "Per-dispatch variety: feature {N}, mean {M}, delta {D} {SURFACED/within-threshold}",  # distribution row; only when coverage >= 50%
  "Coverage: {stamped} of {total} stamped, {malformed} malformed — {reason}",  # ALWAYS when surfaced, at any coverage
...
The `Per-dispatch variety` DISTRIBUTION row is omitted when `coverage < 0.5`, because a mean over almost no stamps is not informative. That threshold governs the distribution only: the `Coverage` row carries every `surfaced=True` finding at ANY coverage, so a zero-coverage session and a session with malformed stamps still reach the record — those are the findings the distribution row cannot express, not findings to drop. the `Variety acknowledgment` decision row is appended only when question 6's dual-trigger fired. The `variety_acknowledgment` and `cargo_cult_signal` entities are added only when question 6 surfaces.
