...
**Sample output for question 5 (variety divergence)** — the ORDINARY case, where stamps exist and a distribution can be computed:
```
...
**Sample output when nothing was stamped** (`reason="zero_coverage"`, `surfaced=True`). `mean`, `min` and `max` are `None` here because no stamp exists to average, so the distribution line is REPLACED rather than filled with nulls:
```
**Variety divergence** (question 5):
- Coverage: 0 of 5 dispatches stamped, 0 malformed
- No per-dispatch distribution: nothing was stamped, so mean/min/max do not exist
- Every dispatch site was recorded and none carried a variety stamp
```

**Sample output when no dispatch site was recorded** (`reason="no_dispatch_sites"`, `surfaced=False`) — report N/A, never a zero:
```
