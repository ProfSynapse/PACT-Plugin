...
and print no line a form does not list.
...
**NO SAMPLES** — the arc recorded no dispatch sites, so there is no distribution and no delta. This is
not a gap in the metric; it is a session that dispatched nothing:
```
