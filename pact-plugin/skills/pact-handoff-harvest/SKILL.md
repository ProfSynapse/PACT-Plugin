...
2. **`latest_by_ts_then_journal_order`**, defined HERE because it is defined at no site in the repo. It takes the events of ONE group, in journal order, and returns the authoritative one:
   ...
   🔴 **THE STRING COMPARE ON `ts` IS CORRECT BY A CALLER PROPERTY, NOT BY A SCHEMA PROPERTY, AND THE DIFFERENCE IS THE WHOLE OF THE RISK.** `make_event` stamps `%Y-%m-%dT%H:%M:%SZ`, and neither `agent_handoff` emit path passes a `ts` of its own, so each event of this family carries ONE format and a string compare orders them correctly TODAY. `make_event` HONOURS a caller-supplied `ts` through `setdefault`, so the one-format property belongs to the CALLERS and the schema does not enforce it. A second format defeats the compare in silence: `canonical_since()` emits an offset form, and a MIXED SET IS THEN ORDERED BY BYTES AND NOT BY TIME. The error runs in EITHER direction, because a string compare walks left to right and the datetime prefix decides before the suffix does. For two spellings of ONE instant, `+` at 0x2B sorts before `Z` at 0x5A and the offset form reads as OLDER. For a non-UTC offset naming that same instant, the hour differs and it reads as NEWER. Either way the newest write can lose. **If any caller ever passes a `ts`, parse the two forms before you compare them.** A sibling resolver in this repo carries the same warning for the same cause.
