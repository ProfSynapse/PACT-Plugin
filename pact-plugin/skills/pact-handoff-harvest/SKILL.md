...
- **Prune complete `## team=` sections** in `session_processed_tasks.md`, in the agent-memory directory the platform gave you. The file is shared by every secretary instance across every project, so a section may be removed on exactly two grounds and no other: (a) it is your own team's section, or (b) you have VERIFIED that its team is complete by reading that session's journal. Age never prunes: a section's `Last processed` timestamp, however old, is not a ground for removal, and a section with no `Last processed` line is unjudgeable, not stale — a criterion that cannot be evaluated never does. To verify completion, take `{project}` and `{session_id}` from the section header `## team={team_id} ({project}, session {session_id})`; the session directory is `{config_dir}/pact-sessions/{project}/{session_id}` (`{config_dir}` is `$CLAUDE_CONFIG_DIR` when set and non-empty, otherwise `$HOME/.claude`). If the header carries no project or session id, or `{that dir}/session-journal.jsonl` does not exist, the team cannot be verified: the section stays. Otherwise run `python3 "{plugin_root}/hooks/shared/session_journal.py" read-last --session-dir '{that dir}' --type session_end`, then the same command with `--type session_paused`, then with `--type session_start` (`read-last` prints the newest event of that type as one JSON object, or `null`). The team is verified complete only when at least one of `session_end` / `session_paused` is non-null AND the later of their `ts` values is later than the `ts` of the `session_start` event (or `session_start` is `null`). A `session_start` later than the newest completion event means the session came back after closing, whatever its `source`: the team is live and the section stays. `null` from both completion reads means not complete; a non-zero exit or non-JSON output means the check could not run. In every case but a verified completion, the section stays. Removing a paused team's section is safe — a pruned-then-resurrected team re-derives its processed set by re-running Step 1 discovery over its own session — so do not keep a section because the session might resume. Re-deriving from `agent_handoff` events alone under-counts the processed set, because tasks found only by the metadata census emit no such event; the save-vs-update dedup absorbs the re-read. Before removing anything, report what you would remove: each section header with its byte size, then the section count and byte total. Then remove those sections and nothing else. (Pruning happens only in this deep-clean Consolidation pass — the Standard/Incremental hot paths leave the file untouched apart from your own section.)
