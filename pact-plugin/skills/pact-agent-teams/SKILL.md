...
6. Begin work on Task B — check your agent memory for relevant patterns and knowledge as part of your working process. The platform hands you its absolute path under `~/.claude/agent-memory/`, and the schema for writing to it, in your own context — follow that instruction rather than any pattern restated elsewhere.
