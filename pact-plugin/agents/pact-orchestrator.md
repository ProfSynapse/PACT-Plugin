...
- After CODE phase completes, once no auditor task is `in_progress` → Standard Harvest (`orchestrate`)
...
- After every reviewer has reported → Standard Harvest (`peer-review`) (**PRIMARY trigger**, fires unconditionally). Waiting until reviewers report means a session dying mid-review has not yet banked those HANDOFFs; the Consolidation safety net below recovers them.
- After remediation completes → Incremental Harvest (`peer-review`) (delta only, only if remediation occurred)
- After comPACT specialists complete, once no auditor task is `in_progress` → Standard Harvest (`comPACT`)
...
Never fire a phase-boundary harvest (Standard, Incremental) while an agent whose value is independent judgement still has that judgement in flight — a reviewer whose HANDOFF has not arrived, or an auditor whose task is still `in_progress`. The harvest writes to the `CLAUDE.md` Working Memory block, and the platform pushes that block into every live agent's context.
