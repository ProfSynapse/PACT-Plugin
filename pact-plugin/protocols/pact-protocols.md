...
The wrap-up retrospective's Q5 (variety divergence) reports `coverage = (stamped dispatches) / (Task-B dispatch sites)`; coverage below 1.0 means at least one Task-B dispatch was not variety-stamped. BOTH terms are derived by the pure helper `extract_dispatch_coverage` from a single read of the `dispatch_site` journal stream, scoped to the current arc:

- The DENOMINATOR is the EXISTENCE of each `dispatch_site` event — one per Task-B dispatch site, emitted whether or not that dispatch carried a variety stamp.
- The NUMERATOR is the count of those same events carrying a RESOLVABLE `variety`, resolved through the shared `resolve_variety_total` accessor (so a stamp recovered via a non-canonical candidate still counts as stamped).
- A stamp that is PRESENT but unresolvable is reported separately as malformed. It is NOT the same finding as an ABSENT stamp — the first is a data-quality defect, the second is a genuine coverage gap, and they have different remedies.

Because every stamped event also incremented the denominator, the two terms cannot be sourced over different populations and `coverage > 1.0` is structurally impossible rather than merely unobserved. Zero coverage has TWO opposite meanings and the DENOMINATOR distinguishes them, never the `coverage` float, which is `0.0` in both: `no_dispatch_sites` (total 0) means nothing was dispatched, so there is no entity to measure — `surfaced=False`, reported as N/A and never as `0.000`, because printing a confident zero for it manufactures a compliance gap out of a missing stream; `zero_coverage` (total > 0, nothing stamped) means every dispatch site exists and none was stamped, which is `surfaced=True` because it is the loudest legitimate reading of this metric and suppressing it would be an invisible refusal reading as "nothing to report". The consumer MUST honour `surfaced` and render the raw `stamped`/`total` counts rather than the ratio alone.

Un-stamped reuse dispatches COUNT in the denominator: there is no variety-exemption carve-out (every Task-B work task is variety-eligible), so an under-stamping gap legitimately lowers coverage — exactly what coverage exists to surface. Teachback Task-A gates and teachback-exempt owners are absent from the denominator because they are not dispatch SITES — and that category does not rest on "because we do not count them", which would be exclusion by fiat. It rests on independent ground that predates this metric: `TEACHBACK_EXEMPT_AGENT_TYPES` is a pre-existing protocol declaration that those owners are dispatched WITHOUT the Task-A gate, and the gate is what makes a dispatch a dispatch. The category exists in the protocol BEFORE the denominator consults it, which is what makes this a category judgment rather than number management — and it is checkable by a reader who does not trust us, because the constant was written for a different purpose. So they fail the question "is this a dispatch requiring understanding verification?", which is a DIFFERENT question from "was this dispatch stamped?". Keep the two apart: a teachback-exempt owner is not a variety-eligible DISPATCH, which is not the same thing as exempting a dispatch from stamping, and nothing here licenses the second. NEVER add an exclusion because a coverage number looks bad — an owner or dispatch shape that is not being stamped is a gap whose remedy is stamping it, never removing it from the denominator. Because the platform reuses task_ids across arcs in a resumed session, the `dispatch_site` stream MUST be scoped to the current arc before counting. `compute_variety_divergence` still returns `reason="coverage_exceeds_unity"` (an advisory, not a clamp) if the stamped count ever exceeds the total, but that condition is now unreachable from Q5 by construction; it is retained only for a caller that assembles the two terms by hand.
