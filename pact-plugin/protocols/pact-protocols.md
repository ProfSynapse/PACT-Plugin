...
**Terms.** One pass of the pure helper `extract_final_dispatch_coverage` over the `dispatch_site` stream and the `task_metadata_snapshot` stream returns a dict of five values. MEMBERSHIP comes from `dispatch_site` and the VALUE comes from the latest `task_metadata_snapshot` for the same task, so the distribution holds the FINAL total rather than the as-dispatched one:
...
- `fallback_used` — the number of members for which the final value did NOT come from a snapshot. Report the count whenever it is non-zero, and do not compute a rate across it.
- `superseded` — the number of members for which the final value DIFFERS from the as-dispatched one, counted only where the two values each resolve. It reports if the stale-value defect continues to occur. Report the count whenever it is non-zero.
