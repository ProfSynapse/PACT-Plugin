...
**Terms.** One pass of the pure helper `extract_dispatch_coverage` over that stream returns three values:

- `variety_totals` — the resolved total of every event carrying a RESOLVABLE `variety`, via the shared `resolve_variety_total` accessor (so a stamp recovered through a non-canonical candidate still counts). This list IS the distribution the delta is computed over.
...
- `malformed` — events whose `variety` is PRESENT but unresolvable. A data-quality defect, NOT the same finding as an ABSENT stamp, and never merged with one: the remedies are opposite.
